					...
					<div class="CenterRightSide">
					  ...
					  <div class="BorderClass Center">
							...
							<div class="BorderClass HolderContent" <%'=shoppingCartShow()%> >
								<div class="LeftBar">
									<div class="RightBar">
										<div class="TitleBar">Các sản phẩm đã đặt</div>		
									</div>
								</div>
									...
									<div class="InfoContent">									
										<div class="InfoBar">
											...
											<div class="LabelBar" onclick="funClickShowHide('ShoppingCartFull')">
													<a id="shoppingCartView" name="shoppingCartView"><span></span></a>
													Giỏ hàng của bạn 											
													<% if session("dadangky")=true then%> 
														:<%=session("tendangnhap")%>	-:- <a href="huybo.asp">Hủy bỏ việc mua hàng</a> 
													<%end if%>		  
											</div>
										</div>
										<div id="ShoppingCartFull" class="OverFlowHidden">
										<!--Begin Shopping content-->	
														<!--#include file="executeData/shoppingCartFunction.asp" -->
														<%
															If Trim(Request.QueryString("option")) = "add" and Trim(Request.QueryString("pID")) <> "" Then
																pAdd Trim(Request.QueryString("pID"))
															End If
															
															If Trim(Request.QueryString("option")) = "remove" and Trim(Request.QueryString("pID")) <> "" Then
																delOne Trim(Request.QueryString("pID"))
															End If
															
															If Trim(Request.QueryString("option")) = "clear" Then
																delAll
															End If
															
															If Trim(Request.QueryString("option")) = "update" Then
																UpdateForm
															End If
														%>
														
														<center>
															<form id="shoppingCart" name="shoppingCart" action="defaultFull.asp">	
																<table width="auto" border="1" cellpadding="0" cellspacing="0" bordercolor="#CCCCCC">
																	<tr align="center">
																		<th width="65" >Mã số </th>
																		<th width="148" >Tên sản phấm </th>
																		<th width="84" >Đơn giá </th>
																		<th width="95" >Số lượng </th>
																		<th width="97" >Thành tiền </th>
																		<th width="22" >&nbsp;</th>
																	</tr>
																	<%
																	If isarray(Session("prdct")) Then
																		If uBound(Session("prdct")) >= 0 Then																	
																			strSql = "Select * from hanghoa where mahang = "
																			For i = 0 to uBound(Session("prdct"))
																				strSql = strSql & Session("prdct")(i)
																				If i <> uBound(Session("prdct")) Then
																				strSql = strSql & " or mahang = "
																				End If																
																			Next															
																			rs.open strSql, conn																
																			Sumary = 0
																					While not rs.eof
																						total = Clng(rs("dongia")*pQty(rs("mahang")))
																	%>
																	<tr>
																		<td ><%=rs("kyhieu")%></td>
																		<td ><%= rs("tenhang") %></td>
																		<td ><%= rs("dongia") %></td>
																		<td >
																			<input name="qty_<%=rs("mahang")%>" type="text" class="formItem" value="<%=pQty(rs("mahang"))%>" size="1" maxlength="2" />
																	
																		</td>
																		<td ><%= total %></td>
																		<th >
																			<a href="javascript:submitActionForm('shoppingCart','remove','product','<%=rs("mahang")%>');">Xóa</a>													
																		</th>
																	</tr>
																	<%
																				Sumary = Sumary + total
																	...
																	<tr >
																		...
																		<th align="right" >&nbsp;<%session("tongtien")= Sumary %><%= Sumary %></th>
																		<td >Vnđ</td>
																	</tr>	
																	<tr>
																		<td colspan="6">
																						...
																						<div class="ButtonLeft">
																							<div class="ButtonRight">
																								<div class="ButtonMiddle">
																									...
																									<a href="javascript:submitActionForm('shoppingCart','update','shoppingCart','');">
																										Hoàn tất 
																									</a>																	
																								</div>
																							</div>
																						...
																						<div class="ButtonLeft">
																							<div class="ButtonRight">
																								<div class="ButtonMiddle">
																									...
																									<a  href="javascript:submitActionForm('actionForm','regester','shoppingCart','');" title="Hoàn tất">
																										Đặt mua 
																									</a>																	
																								</div>
																							</div>
