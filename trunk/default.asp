					...
					<div class="CenterRightSide">
					  ...
					  <div class="BorderClass Center">
						    ...
						    <!-- InstanceBeginEditable name="inhatl contents" -->
								...
								<div class="LeftBar">
									<div class="RightBar">
										<div class="TitleBar" onclick="funClickShowHide('BodyCenterDetail');">Nội dung</div>																	
									</div>
								</div>
								<div id="BodyCenterDetail" class="Detail">
									<div class="OverFlowHidden">
										<!-- begin best Sale -->
										<div class="BorderClass InfoContent">
											<div class="InfoBar">
												<div class="LeftBlock Icon"><span></span></div>
												<div class="LabelBar" onclick="funClickShowHide('ProductHolder')" >Sản phẩm bán chạy nhất</div>
											</div>
											 
												<div id="ProductHolder" class="BorderClass ProductHolder">
														
													<%													
													
													sqlmoi="select TOP 4 * from hangbanchay "
													rs.open sqlmoi,conn 
													while not rs.eof
													
													%>							
														<div class="BorderClass LeftBlock Product">													
															<div class="BorderClass  LeftBlock ProductImage">													
																<a  href="javascript:submitActionForm('actionForm','view','product','<%=rs("mahang")%>');" title="Xem chi tiết">
																<img  border="none" width="80px" height="80px"  src="<%=rs("anh")%>"/>														</a>
															</div>	
															<a href="javascript:submitActionForm('actionForm','view','product','<%=rs("mahang")%>');"><%=rs("tenhang")%></a>:
															<% = left(rs("hhmt"),150)%>...
															<div class="ClearAll"><span></span></div>
															<div class="BorderClass LeftBlock Prices"><%=rs("dongia")%>:đ</div>
															<div class="ProductQuick BorderClass">
																<a href="javascript:submitActionForm('shoppingCart','add','product','<%=rs("mahang")%>');">Đặt mua </a>
															</div>															
														</div>
													<%
							...
							<!--begin tin khuyen mai-->		
							<div class="OverFlowHidden">
								
									<div class="InfoContent">									
										<div class="InfoBar">
											<div class="LeftBlock Icon"><span></span></div>
											<div class="LabelBar" onclick="funClickShowHide('InsideNew')">Tin khuyến mại</div>
										</div>
										<% 											
											if((Request.Form("type")="news")and(Request.Form("actionType")="view")) then
											sqlstringselect =  "select  *  from tintuc where matintuc = "& Request.Form("itemId") 
												rs.open sqlstringselect  , conn
													if not rs.eof then %>																				
														<div class="BorderClass ">
															<div class="ProductNewName"><%=rs("tieude")%></div>
															<div class="BorderClass LeftBlock ProductNewImage">
																<a  name="news<%=rs("matintuc")%>" id="news<%=rs("matintuc")%>" >
																	<img  border="none"  src="<%=rs("anhdikem")%>" width="130px" height="130px" />
																</a>
															</div>
															<div class="BorderClass">
										...
										<div class="ClearAll"><span></span></div>
										<div id="InsideNew" class="BorderClass ProductHolder">														
											<% 
												selectnew =  "select Top 5 * from tintuc order by ngaygui desc "
												if Request.Form("itemId") > 0 then
											      selectnew =  "select Top 5 * from tintuc where matintuc <> "&Request.Form("itemId") & " order by ngaygui desc"
												end if  
												rs.open selectnew , conn
											%>														
											<div class="BorderClass RelativePro">
											 <%while not rs.eof %>
												<div class="RelativeProName">
													-<a  href="javascript:submitActionForm('actionForm','view','news','<%=rs(0)%>');" title="Xem chi tiết">
														<%=rs("tieude")%> 
													 </a>
												</div>
											 <%
