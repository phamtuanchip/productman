					...
					<div class="CenterRightSide">
					  ...
					  <div class="BorderClass Center">
						    ...
						    <!-- InstanceBeginEditable name="inhatl contents" -->
								<% 
									sqlstring = "select * from chungloai"
									if Request.Form("itemId") > 0 then 
										sqlstring = "select * from chungloai where maloai="&Request.Form("itemId")
									end if
								...
								<div class="LeftBar">
									<div class="RightBar">
										<div class="TitleBar" onclick="funClickShowHide('BodyCenterDetail');">Danh mục các loại sản phẩm</div>																	
									</div>
								</div>
								<div id="BodyCenterDetail" class="BorderClass InfoContent">
									<!-- Begin Categories -->
																										
									<% while not rs.eof %>	
										<div class="OverFlowHidden">																		
											<div class="InfoBar">
												<div class="LeftBlock  BorderClass Icon"><span></span></div>
												<div class="LabelBar">
													<div class="BorderClass LeftBlock  HomeIcon" title="Về đầu trang" onclick="goTo('#');"><span></span></div>
													<a name="category<%=rs("maloai")%>" id="category<%=rs("maloai")%>" onclick="funClickShowHide('CategoryAll<%=rs("maloai")%>')"><%=rs("tenloai")%></a>																						
												</div>											
											</div>										
											<div  id="CategoryAll<%=rs("maloai")%>" class="BorderClass CategoryAll">
												<div class="OverFlowHidden">
													<%dim rshh
														set rshh = server.CreateObject("adodb.recordset")
														rshh.open "Select * from hanghoa where chungloai="&rs("maloai"), conn 
														while not rshh.eof
													%>
														<div class="LeftBlock CateGoryItem BorderClass">
															<div class="BorderClass CateGoryItemName">
																<a  href="javascript:submitActionForm('actionForm','view','product','<%=rshh("mahang")%>");" title="Xem chi tiết">
																
																	<%=rshh("tenhang")%>
																</a>
															</div>
	
															<a  href="javascript:submitActionForm('actionForm','view','product','<%=rshh("mahang")%>');" title="<%=left(rshh("mota"),100)%>...">
																<img border="none" src="<%=rshh("anh")%>" width="120px" height="120px"/>
															</a>
															
															<div class="CateGoryQuickBook BorderClass">
																<div class="LeftBlock Ranking<%=(rshh("luotxem")mod 6)%>"><span></span></div>
																<a href="javascript:submitActionForm('shoppingCart','add','product','<%=rshh("mahang")%>');">Đặt mua </a>
															</div>
