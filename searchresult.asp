					...
					<div class="CenterRightSide">
					  ...
					  <div class="BorderClass Center">
							...
							<div class="BorderClass HolderContent" <%'=shoppingCartShow()%> >
								<div class="LeftBar">
									<div class="RightBar">
										<div class="TitleBar">Kết quả tìm kiếm của bạn</div>		
									</div>
								</div>
								<div class="InfoContent" <%'confirmShow()%> >
									<% 
										options = Request.Form("searchoption")
										values = Request.Form("searchvalue")
										values = lcase(replace(values,"'","")) 
										dim rscount
										set rscount = server.CreateObject("adodb.recordset")
										itemcount = 0 
										searchquery = "select * from hanghoa where tenhang like '%"&values&"%' or mota like '%"&values&"%' or hangsx like '%"&values&"%'"
										countquery = "select count(mahang) from hanghoa where tenhang like '%"&values&"%' or mota like '%"&values&"%' or hangsx like '%"&values&"%'"
										if cint(options) <> 0  then
											searchquery = "select * from hanghoa where chungloai = "&options & "and (tenhang like '%"&values&"%' or mota like '%"&values&"%' or hangsx like '%"&values&"%')"
											countquery = "select count(mahang) from hanghoa where chungloai = "&options & "and (tenhang like '%"&values&"%' or mota like '%"&values&"%' or hangsx like '%"&values&"%')"
										end if
										rscount.open  countquery, conn
											itemcount = rscount(0) 
										rscount.close()
									...
									<div class="InfoBar">
										...
										<div class="LabelBar" onclick="">Tìm thấy (<%=itemcount%>) kết quả !</div>
									</div>
